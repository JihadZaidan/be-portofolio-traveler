...
export class AutoChatController {
  ...
  private travelKnowledgeBase: Map<string, string[]>;
   ...
   */
  private initializeTravelKnowledgeBase(): void {
    this.travelKnowledgeBase = new Map([
      ['wisata', [
   ...
   */
  private generateAutoResponse(message: string, history: any[]): string {
   ...
   */
  private formatTravelResponse(response: string, category: string): string {
    ...
    const emoji = categoryEmojis[category as keyof typeof categoryEmojis] || '✈️';
   ...
   */
  private generateRecommendationResponse(message: string, history: any[]): string {
    const recentTopics = history.slice(-3).map(msg => 
      (msg.message || msg.response).toLowerCase()
    ).join(' ');
  ...
  async getSuggestions(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const suggestions = this.generateContextualSuggestions(
        recentHistory[recentHistory.length - 1]?.response || ''
      );
