...
class ImprovedChatController {
  ...
  async chat(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const currentSessionId = sessionId || `session_${mockUser.id}_${Date.now()}`;
      ...
      if (dbHistory.length > 0) {
        ...
        formattedHistory = this.geminiService.formatHistory(
          history.map(msg => ({
  ...
  async chatStream(req: Request, res: Response): Promise<void> {
    try {
      ...
      try {
        // Streaming response
        for await (const chunk of this.geminiService.chatWithGeminiStream(message, formattedHistory)) {
          fullResponse += chunk;
          res.write(chunk);
        }

        // Simpan pesan user dan AI response ke database setelah streaming selesai
        await createMessage({
          sessionId: currentSessionId,
          userId: mockUser.id,
          message: message,
        ...
        console.log(`[${new Date().toISOString()}] Stream Chat Success:`, {
          userId: mockUser.id,
          sessionId: currentSessionId,
          messageLength: message.length,
        ...
        res.write('\n\n❌ Terjadi kesalahan saat streaming. Silakan coba lagi.');
      }
      ...
      if (!res.headersSent) {
        res.status(500).json({
          success: false,
          error: 'Gagal memulai streaming. Silakan coba lagi.'
        });
      }
    }
  }
   ...
   */
  private generateSuggestions(history: any[]): string[] {
